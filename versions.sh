#!/usr/bin/env bash
{
	command -v lsb_release >/dev/null && echo -e "$(lsb_release --short --description):$(lsb_release --release --short)"
	command -v cygcheck >/dev/null && echo -e "Cygwin:$(cygcheck --version | head -n 1 | grep -o '[^ ]*$')"
	...
	command -v python3 >/dev/null && echo -e "Python3:$(python3 --version 2>/dev/null || echo "?" | cut -d' ' -f2)"
